...
const double K=50;
const double E=1.0, r0=10, kbT=0.1;
const double Lx=140, Ly=140;
...
public:
  ...
  void CalculeLaFuerzaEntreGranoyPared(Cuerpo & Grano1, Cuerpo & Grano2);
};
...
void Colisionador::CalculeTodasLasFuerzas(Cuerpo* Grano){
  ...
  for(i=0;i<N;i++){
    for(j=N;j<N+4;j++){
      CalculeLaFuerzaEntreGranoyPared(Grano[i], Grano[j]);
    }
  }
}
...
void Colisionador::CalculeLaFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2){
  ...
  if(s>0){
    F2=runitario*(12*E/distancia*(pow((r0/distancia),12)-pow((r0/distancia),6)));
    Grano1.AgregueFuerza(F2*(-1)); Grano2.AgregueFuerza(F2);
  }
}

void Colisionador::CalculeLaFuerzaEntreGranoyPared(Cuerpo & Grano1, Cuerpo & Grano2){
  vector3D F2, runitario, dr = Grano2.r-Grano1.r;
  double s, distancia=norma(dr);
  runitario=dr/distancia;
  s=Grano1.R+Grano2.R-distancia;
...
void InicieAnimacion(void){
  ...
  cout<<"set output 'gasLennardJones.gif'"<<endl;
  cout<<"unset key"<<endl;
  cout<<"set xrange [-40:100]"<<endl;
  cout<<"set yrange [-40:100]"<<endl;
...
int main(void){
  ...
  double m0=1.0, R0=2.5, V0=pow(2*kbT/m0,0.5);
  double Rpared=10000, Mpared=1000;
  double tmax=5;

  double dx=10, dy=10, theta;
  ...
  //pared arriba
  Grano[N].Inicie(30, 100+Rpared, 0, 0, 0, 0, Mpared, Rpared);
  //pared abajo
  Grano[N+1].Inicie(30, -40-Rpared, 0, 0, 0, 0, Mpared, Rpared);
  //pared derecha
  Grano[N+2].Inicie(100+Rpared, 30, 0, 0, 0, 0, Mpared, Rpared);
  //pared izquierda
  Grano[N+3].Inicie(-40-Rpared, 30, 0, 0, 0, 0, Mpared, Rpared);
  ...
  for(i=0;i<Nx;i++)
    for(j=0;j<Ny;j++){
      ...
      Grano[i+Nx*j].Inicie((i+1)*dx, (j+1)*dy, 0, V0*cos(theta), V0*sin(theta), 0, m0, R0);
    }
