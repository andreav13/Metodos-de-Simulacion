...
// -------Elastic Moduli.  1/E = (1-v1^2)/E1 + (1-v2^2)/E2------//
...
void Colisionador::CalculeLaFuerzaEntrePlanos(Cuerpo * Grano, double dt){
  ...
  //----------------------------------------------------------------------------------------
  //Colision entre los vertices del cubo y la superficie. Tipo de colision: Esfera-Esfera
  //----------------------------------------------------------------------------------------
  if(cual==0){
  for (int i = 0;i<8;i++){
    double r2=-L/2.; x=Grano[i].Getx(); y=Grano[i].Gety(); z=Grano[i].Getz()-R;
    if(z<r2){        //Condicion de contacto Esfera-Superficie.
  ...
  //----------------------------------------------------------------------------------
  //Colision entre las caras del cubo y la superficie. Tipo de colision: Plano-Plano
  //----------------------------------------------------------------------------------
  if (cual==1){
  for (int i = 20;i<N-1;i++){
  ...
  //------------------------------------------------------------------------------------------------
  // Colision entre los lados del cubo y la superficie cilindro. Tipo de colision: Cilindro-Cilindro
  //------------------------------------------------------------------------------------------------
  if(cual==2){
  ...
  for (int i = 8;i<20;i++){
    ...
    if(zeta<r2){               //Condicion de contacto Cilindro-Superficie.

      double s_cilindro=r2-z;  //Distancia que penetra el cilindro en el plano(otro cilindro)                                    
  ...
  //Calculo del torque
  vector3D torque,cero; cero.cargue(0,0,0);
...
int main(void){
  ...
  //Dibuja cubo y plano para cada paso de tiempo
