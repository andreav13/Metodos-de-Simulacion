...
public:
  ...
  double rho(int ix, int iy, bool UseNew, double sigma, double t);
...
double LatticeBoltzmann::rho(int ix, int iy, bool UseNew, double sigma, double t){
  double A=1, lambda=1000, omega=2*M_PI*C/lambda, suma; int i;
  if(ix==0)
...
void LatticeBoltzmann::Colisione(int t){ //de f a fnew
  ...
  for(ix=0;ix<Lx;ix++)
	...
	rho0=rho(ix,iy,false,sigma,t); Jx0=Jx(ix,iy); Jy0=Jy(ix,iy);
...
void LatticeBoltzmann::Imprimase(char const * NombreArchivo, int t){
  ...
  for(int ix=0;ix<Lx;ix++){
    for(int iy=0;iy<Ly;iy++){
      ...
      rho0=rho(ix,iy,true,sigma,t); Jx0=Jx(ix,iy);  Jy0=Jy(ix,iy);
...
void LatticeBoltzmann::ImprimaUnaLinea(char const * NombreArchivo,int t){
  ofstream MiArchivo(NombreArchivo, ios::out | ios::app); double rho0,Jx0,Jy0;
  int ix=Lx/2;
  for(int iy=0;iy<Ly;iy++){
    double sigma = GetSigma(ix,iy,t);
    rho0=rho(ix,iy,true,sigma,t);   Jx0=Jx(ix,iy);  Jy0=Jy(ix,iy);
...
int main(void){
  ...
  Ondas.Imprimase("densidad.dat", t);
