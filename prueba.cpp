...
int main(void){
  ...
  ofstream RhoMax("rhomax50.dat");
  ofstream RhoMin("rhomin50.dat");
  int ix=50, iy=0;
  double rhomax=0,rhomin=0;
  for(t=tmax;t<tmax+200;t++){
    Ondas.Colisione(t);
    Ondas.Adveccione();
    
    double sigma = Ondas.GetSigma(ix,iy,t);
    rho0=Ondas.rho(ix,iy,true,sigma);
    if (rho0<rhomin) rhomin=rho0;
    if (rho0>rhomax) rhomax=rho0;

    RhoMax<<t<<" "<<rho0<<endl;
  }
  //RhoMax<<ix<<" "<<rhomax<<endl;
